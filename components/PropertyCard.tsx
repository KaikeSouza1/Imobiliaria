...
export default function PropertyCard({ property }: PropertyProps) {
  ...
  const statusConfig: Record<string, { label: string; bgClass: string; bgHex: string; overlay: string; icon: React.ReactNode }> = {
    vendido: {
      label: "VENDIDO",
      bgClass: "bg-[#0f2e20]",
      bgHex: "#0f2e20",
      overlay: "bg-[#0f2e20]/70",
  ...
  return (
    <div className="rounded-[2rem] overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-green-900/30 transition-all duration-500 group border border-green-900/10">
      ...
      <Link href={`/imovel/${property.id}`} className="relative block h-64 overflow-hidden">
        ...
        {/* FAIXA DIAGONAL — agora em verde escuro */}
        {isSoldOrRented && (
        ...
        <div className="absolute top-4 left-4">
          <span
            className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest text-white shadow-lg backdrop-blur-sm"
            style={{ backgroundColor: currentStatus.bgHex }}
          >
        ...
        <div className={`absolute bottom-4 right-4 backdrop-blur-sm px-4 py-2 rounded-2xl shadow-lg ${isSoldOrRented ? "bg-[#0f2e20]/80" : "bg-white/90"}`}>
          <p className={`text-[10px] font-bold uppercase tracking-tighter leading-none ${isSoldOrRented ? "text-green-300" : "text-gray-400"}`}>
            Valor
          </p>
      ...
      {/* ── CONTEÚDO COM DEGRADÊ VERDE BEM PRESENTE ── */}
      {/*
        Degradê: branco no topo → verde claro no meio → verde escuro da marca no fundo
        O z-10 garante que o texto fica sempre na frente
      */}
      <div className="relative overflow-hidden">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(180deg, #ffffff 0%, #a8d5b5 40%, #0f2e20 100%)",
          }}
        />
        ...
        <div className="relative z-10 p-6">
          ...
          {/* LOCALIZAÇÃO — branco pois já está sobre fundo médio */}
          ...
          {/* TÍTULO */}
          <h3 className={`text-xl font-bold mb-4 line-clamp-1 transition-colors
            ${isSoldOrRented ? "text-green-900/60" : "text-gray-800 group-hover:text-[#0f2e20]"}`}>
            {property.titulo}
          </h3>

          {/* BANNER STATUS em verde */}
          {isSoldOrRented && (
            <div
              className="text-white text-xs font-black uppercase tracking-widest px-4 py-2 rounded-xl mb-4 flex items-center gap-2"
              style={{ backgroundColor: currentStatus.bgHex }}
            >
              {currentStatus.icon}
