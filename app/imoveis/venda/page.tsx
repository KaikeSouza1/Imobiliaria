...
function ordenarImoveis(lista: Imovel[], ordem: OrdemTipo): Imovel[] {
  ...
  // Move imóveis marcados como vendido/alugado/indisponível para o final,
  // mas preserve a ordenação solicitada entre os demais.
  const isUnavailable = (i: Imovel) => {
    const s = (i.status || "").toLowerCase();
    return s.includes("vendido") || s.includes("alugado") || s.includes("indisponível");
  };
...
function VendaContent() {
  ...
  useEffect(() => {
    if (!loading) {
      try {
        const key = `listingState:/imoveis/venda`;
  ...
  const imoveisFiltrados = useMemo(() => {
    const filtrados = imoveis.filter((imovel) => {
      ...
      if (ocultarIndisponiveis) {
        ...
        if (status.includes("vendido") || status.includes("alugado") || status.includes("indisponível")) return false;
      }
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            ...
            >
              ...
              <span className="hidden sm:inline">
                {ocultarIndisponiveis ? "Ocultando Vendidos" : "Ocultar Vendidos"}
              </span>
            </button>
