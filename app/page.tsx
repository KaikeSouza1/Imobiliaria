...
import { useRouter } from "next/navigation"; // Para fazer o redirecionamento da busca
...
export default function Page() {
  ...
  // Estados para armazenar os imóveis reais
  const [imoveis, setImoveis] = useState<Imovel[]>([]);
  const [loading, setLoading] = useState(true);

  // Estados do Filtro de Busca
  const [filters, setFilters] = useState({
    finalidade: "Comprar", // Padrão
    tipo: "",
    cidade: "",
    bairro: "",
    codigo: ""
  ...
  // 1. BUSCAR IMÓVEIS DO BANCO DE DADOS
  useEffect(() => {
    async function fetchImoveis() {
      try {
        const res = await fetch("/api/imoveis"); // Chama nossa API
        if (res.ok) {
          const data = await res.json();
          // Filtra apenas os imóveis ATIVOS para mostrar no site
  ...
  // 2. SEPARAR VENDA E ALUGUEL (Pega os 3 mais recentes de cada)
  const destaquesVenda = imoveis
    .filter(i => i.finalidade === "Venda")
    .slice(0, 3); // Mostra só os 3 primeiros na Home

  const destaquesAluguel = imoveis
    .filter(i => i.finalidade === "Aluguel" || i.finalidade === "Locação")
    .slice(0, 3);

  // Função para formatar o preço (De 1000 para R$ 1.000,00)
  ...
  const handleSearch = () => {
    // Se tiver código, prioriza ele
    if (filters.codigo) {
      // Futuramente podemos criar uma página de detalhes direta, por enquanto busca na lista
      router.push(`/imoveis/venda?codigo=${filters.codigo}`);
      return;
    }

    // Define a rota base (Venda ou Aluguel)
  ...
  // Helper para adaptar o objeto do banco para o PropertyCard
  // O PropertyCard espera strings em alguns lugares e o banco manda numeros
  const mapToCard = (imovel: Imovel) => ({
    ...imovel,
    preco: formatMoney(imovel.preco), // Formata o preço
    imagem: imovel.imagem_url || "https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=800" // Fallback se não tiver foto
  });
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans overflow-x-hidden relative">
      ...
      <div className="relative -mt-32 z-20 px-4">
        <div className="max-w-6xl mx-auto bg-white rounded-[2rem] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)] overflow-hidden border border-gray-100">
          ...
          <div className="bg-gray-50 border-t border-gray-100 p-6 flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      {/* === SEÇÃO DE CONTATO (EXCLUSIVA DA HOME) === */}
      <HomeContact />

      {/* === BOTÃO DE ADMINISTRAÇÃO (DISCRETO) === */}
